...
import { dummyRecipesData, Recipe, RecipeIngredient, RecipeStep } from '@/lib/dummy-recipes';
...
const PlateOfTheDayPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = y.on("change", (latest) => {
        if (latest < 0) { // only prevent scroll if dragging down to reveal, not scrolling within content
  ...
  const handleDragEnd = (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    if (info.offset.y > dragThreshold && info.velocity.y > 20) { // Check offset and velocity
      router.push('/recipes');
    } else {
  ...
  return (
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-gray-50">
      <motion.div
        ...
        className="container relative z-10 mx-auto max-w-3xl cursor-grab px-0 active:cursor-grabbing sm:px-2"
      >
        ...
        <div className="bg-gray-50 py-8">
            ...
            <div className="p-6 md:p-8">
                <h2 className="mb-3 text-3xl font-semibold text-gray-900">{recipe.title}</h2>
                <p className="mb-6 text-base text-gray-600">{recipe.description}</p>
                ...
                {cookTime && (
                    <div className="flex items-center">
                    <ChefHat size={18} className="mr-2 text-primary" />
                ...
                <h3 className="mb-3 flex items-center text-xl font-semibold text-gray-800">
                    <ListChecks size={22} className="mr-2 text-primary" /> Ingredients
                </h3>
                <ul className="list-disc space-y-1 pl-6 text-gray-700">
                    {recipe.ingredients.map((ingredient: RecipeIngredient, index: number) => (
                    <li key={index}>
                        {ingredient.quantity} {ingredient.name}
                        {ingredient.notes && <span className="text-xs text-gray-500"> ({ingredient.notes})</span>}
                    </li>
                    ))}
                </ul>
                </section>
                ...
                <section>
                <h3 className="mb-3 flex items-center text-xl font-semibold text-gray-800">
                    <ChefHat size={22} className="mr-2 text-primary" /> Instructions
                </h3>
                <ol className="list-decimal space-y-3 pl-6 text-gray-700">
                    ...
                    <li key={step.stepNumber} className="leading-relaxed">
                        ...
                        <div className="mt-2 overflow-hidden rounded-md">
                            <Image src={step.imageUrl} alt={`Step ${step.stepNumber}`} width={400} height={250} className="pointer-events-none object-cover"/>
                        </div>
                        )}
                    </li>
                    ))}
                ...
                <p className="mt-8 text-xs text-gray-500">
                    Original recipe from: <a href={recipe.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{recipe.sourceUrl}</a>
                </p>
                )}
            </div>
            </article>
