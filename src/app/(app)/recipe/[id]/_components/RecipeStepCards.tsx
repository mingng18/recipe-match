...
export default function RecipeStepCards({ steps, onComplete }: RecipeStepCardsProps) {
  ...
  const handleNext = () => {
    ...
    api.start(i => {
      if (i === currentCardIndexInProps) {
        ...
        setCurrentIndex(prev => Math.max(0, prev -1));
        return { x: (200 + window.innerWidth), rot: 30, scale: 1, config: { friction: 50, tension: 200 }};
      }
    });
  };
  ...
  const handlePrev = () => {
    ...
    console.log("Previous button clicked - not fully implemented for bringing cards back yet");
  ...
  return (
    <div className="flex flex-col items-center w-full relative select-none"> {/* Added select-none */}
      <div className="relative w-full max-w-xs h-96 md:max-w-md md:h-[450px] flex items-center justify-center"> {/* Deck container */}
      ...
      {gone.size === steps.length ? (
        <div className="mt-6 text-center">
            <p className="text-xl font-semibold mb-2">Recipe Steps Completed!</p>
        ...
        <div className="mt-6 flex items-center space-x-4">
          <Button onClick={handlePrev} variant="outline" size="icon" aria-label="Previous Step" disabled={currentIndex >= steps.length -1 && !gone.has(0) }>
            <ArrowLeft />
          </Button>
          <span className="text-sm text-muted-foreground">
             {steps.length - gone.size} / {steps.length} Steps Left
          </span>
          <Button onClick={handleNext} variant="outline" size="icon" aria-label="Next Step">
            <ArrowRight />
          </Button>
        </div>
      )}
