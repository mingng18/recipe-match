...
const DragIndicators = ({
  ...
  const [currentView] = useQueryState<ViewType>(
    "view",
    parseAsStringLiteral(acceptedViews).withDefault("recipe"),
  );
  ...
  const isValid = isValidSwipeDirection(currentView, dragDirection);
  ...
  return (
    ...
    >
      ...
      >
        {isValid
          ? getIndicatorText(currentView, dragDirection)
          : "Can't go this way"}
      </div>
    </motion.div>
  );
...
export default memo(DragIndicators);
